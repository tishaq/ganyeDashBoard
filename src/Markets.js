...
export default function DeviceTickets() {
    ...
    const loadDeviceTickets = async (start, end) => {
        setIsLoader(true);
    ...
    const loadDevices = async () => {
        ...
        try {
            do {
                ...
                nextToken = raw.data.listDevices.nextToken;
                raw.data.listDevices.items.map(value => results.push(value));
            } while (nextToken);
            console.log(results);
            setDevices(results);
                    ...
                    <div className="container-fluid">
                        <center>
                            ...
                            <select className="form-control-sm" style={{
                                ...
                                defaultValue="selected" onChange={e => { setDeviceId(e.target.value); setLoadButtonStatus(false); }}>

                                <option disabled value="selected">--Select Market--</option>

                                {devices.map((agent, index) => <option key={index} value={agent.id}>{agent.deviceName}--{agent.id}</option>)}
                            ...
                            <DateRangePicker startDate={new Date()}
                                ...
                                onApply={(e, p) => {
                                    ...
                                    loadDeviceTickets(p.startDate._d, p.endDate._d);
                            ...
                            <div className="col-lg-12 col-md-12 col-sm-12">
                                <div className="card shadow mb-lg-1">
                                    <div className="card-header py-3"
                                        ...
                                        <h6 className="font-weight-bold text-dark text-center" > Ticket Records {deviceId ? " ( " + deviceId + " )" : ""} </h6>
